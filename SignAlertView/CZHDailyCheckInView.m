...
- (void)setIsCheckIng:(NSInteger)isCheckIng {
    ...
    if (isCheckIng == 0) {
        ...
        self.bottomImage.image = [UIImage imageNamed:@"icon_sign_cannot_sign"];
        self.treasureBoxImage.image = [UIImage imageNamed:self.notOpenImage];
    } else if(isCheckIng == 1) {
        self.maskView.hidden = NO;
        self.bottomImage.image = [UIImage imageNamed:@"icon_sign_can_sign"];
        self.treasureBoxImage.image = [UIImage imageNamed:self.notOpenImage];
...
- (void)setView {
    ...
    //bottomImage.image = [UIImage imageNamed:@"icon_sign_cannot_sign"];
    ...
    //    dayCount.text = @"第一天";
    dayCount.font = CZHGlobelNormalFont(6);
    ...
    // treasureBoxImage.image = [UIImage imageNamed:@"icon_sign_firstday_open"];
    ...
    diamondCount.font = CZHGlobelNormalFont(6);
...
- (void)czh_getData {
    ...
    if (checkCache.count <= 0) {//第一次本地数据是空的，添加一个空的放到数据库
        ...
        dic[@"icon_sign_count"] = @"0";
        dic[@"icon_sign_log"] = @[];
        
        [CZHLocalCacheTool czh_updateCheckCacheWithDataDic:dic];
    } else {
        
        NSArray *array = [checkCache valueForKey:@"icon_sign_log"];
...
- (void)czh_setUpView {
    ...
    backgroundImageView.image = [UIImage imageNamed:@"icon_sign_background"];
    ...
    calendarImage.image = [UIImage imageNamed:@"icon_sign_home_calendar"];
...
//    [backgroundImageView addSubview:titleLabel];
    ...
    continueLabel.frame = CGRectMake(calendarImage.czh_right + CZH_ScaleWidth(6), CZH_ScaleHeight(20) + CZH_ScaleHeight(5), CZH_ScaleWidth(200), CZH_ScaleHeight(12));
    continueLabel.text = @"连续签到0天";
    continueLabel.textColor = [UIColor colorWithRed:255.0 green:255.0 blue:255.0 alpha:0.8];
    continueLabel.font = CZHGlobelNormalFont(6);
    ...
    [closeButton setBackgroundImage:[UIImage imageNamed:@"icon_sign_close"] forState:UIControlStateNormal];
    ...
    [receiveReward setBackgroundImage:[UIImage imageNamed:@"icon_sign_receive_reward"] forState:UIControlStateNormal];
...
//        }];
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"signSuccess" object:self userInfo:@{@"type":@"add",@"changeNum":@(10)}];
...
- (void)czh_checkIn {
    ...
    NSString *sign_count = [signDictionary valueForKey:@"icon_sign_count"];
    
    NSInteger signCount = [sign_count integerValue];
    
    NSArray *array = [signDictionary valueForKey:@"icon_sign_log"];
    ...
    [signDictionary setValue:[NSString stringWithFormat:@"%ld", signCount] forKey:@"icon_sign_count"];
    
    [signDictionary setValue:[templeArray copy] forKey:@"icon_sign_log"];
...
- (void)czh_reloadDta {
    ...
    NSArray *array = signDictionary[@"icon_sign_log"];
    
    self.continueLabel.text = [NSString stringWithFormat:@"连续签到%ld天", [signDictionary[@"icon_sign_count"] integerValue]];
...
- (CZHTreasureBoxView *)czh_quickSetUpViewWithFrame:(CGRect)frame currentDay:(NSInteger)currentDay {
    ...
    boxView.isOpenImage = [NSString stringWithFormat:@"icon_sign_open_%ld", currentDay];
    boxView.notOpenImage = [NSString stringWithFormat:@"icon_sign_close_%ld", currentDay];
