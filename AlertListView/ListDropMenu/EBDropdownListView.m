...
- (void)selectedItemAtIndex:(NSInteger)index {
    ...
    if (index < _dataSource.count) {
        ...
        _selectedItem = item;
//        _textLabel.text = item.itemName;//选中标签后会添加文本到视图框内
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
    ...
    UIImageView *imgv = [[UIImageView alloc] initWithFrame:CGRectMake(17, 0, 111, 30)];
    imgv.image = [UIImage imageNamed:@"challengetip"];
    [cell.contentView addSubview:imgv];
    cell.backgroundColor = [UIColor clearColor];/*背景透明*/
    cell.textLabel.font = _font;
    cell.textLabel.textColor = _textColor;
    cell.textLabel.textAlignment = NSTextAlignmentCenter;
    cell.selectionStyle = UITableViewCellSelectionStyleNone;/*点击选中不显示默认灰色，有效*/
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [tableView deselectRowAtIndexPath:indexPath animated:NO];/*点击选中不显示默认灰色，貌似无效*/
...
- (UITableView*)tbView {
    if (!_tbView) {
        ...
        _tbView.backgroundColor = [UIColor clearColor];/*背景透明*/
        _tbView.separatorStyle = UITableViewCellAccessoryNone;/*去分隔线*/
